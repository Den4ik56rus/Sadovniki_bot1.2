...
def get_base_system_prompt_minimal(
    ...
    """
    Возвращает МИНИМАЛЬНУЮ базовую часть системного промпта (БЕЗ инструкций по формату ответа).

    Используется для категорий с детальными промптами (например, клубника/малина в питании),
    где формат ответа уже описан в категорийном промпте.
