...
1. Ответ должен быть КРАТКИМ и КОНКРЕТНЫМ — основные рекомендации без лишних подробностей.
2. Без длинных вступлений и предисловий. Сразу к сути.
3. Дай конкретные рекомендации с названиями препаратов/удобрений и краткими дозировками.
4. Если уместно — краткое обоснование (1-2 предложения).
5. Максимальный объём: до 4000 символов (одно сообщение Telegram). Старайся уложиться в 200-300 слов.
6. Формат: нумерованный список или короткие абзацы.
7. Завершающая фраза: предложи узнать подробнее или выстроить систему обработок/подкормок."""
...
ГЛАВНОЕ ПРАВИЛО: Максимально используй информацию из базы знаний (скрипты, документы, Q&A), которая загружена выше.
Бери из материалов конкретные препараты, дозировки, сроки, схемы обработок для этой фазы.
...
2. Далее — ПОДРОБНЫЙ план работ для этой фазы, опираясь на загруженные материалы.
3. Конкретика: названия препаратов, дозировки, сроки, интервалы.
4. Объясни почему и укажи частые ошибки.

МИНИМУМ 500 слов. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать меньше 3500 символов."""
...
ГЛАВНОЕ ПРАВИЛО: Максимально используй информацию из базы знаний (скрипты, документы, Q&A), которая загружена выше.
Бери из материалов конкретные препараты, дозировки, сроки, схемы обработок для выбранной фазы.
...
Пользователь ЗАПЛАТИЛ за РАСШИРЕННЫЙ ответ. Он ожидает ДЕТАЛЬНУЮ, ПОДРОБНУЮ информацию.

ГЛАВНОЕ ПРАВИЛО: Максимально используй информацию из базы знаний (скрипты, документы, Q&A), которая загружена выше.
Раскрой тему ПОЛНОСТЬЮ, опираясь на загруженные материалы — конкретные препараты, дозировки, сроки, схемы.

ТРЕБОВАНИЯ:
1. Дай ПОДРОБНЫЙ ответ с конкретикой: названия препаратов, точные дозировки, сроки применения.
2. Объясни ПОЧЕМУ — агрономическое обоснование из базы знаний.
3. Приведи альтернативные варианты (если есть в материалах базы знаний).
4. Укажи частые ошибки и предостережения.
5. Структурируй с подзаголовками и списками.
6. МИНИМУМ 500 слов. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать меньше 3500 символов."""
