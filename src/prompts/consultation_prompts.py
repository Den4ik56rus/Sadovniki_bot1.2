...
ИНСТРУКЦИЯ ПО ФОРМАТУ ОТВЕТА — СТАНДАРТНЫЙ (КРАТКИЙ):
Пользователь запросил СТАНДАРТНЫЙ ответ. Строго следуй правилам:
1. Ответ должен быть ОЧЕНЬ КРАТКИМ — максимум 3-4 коротких пункта.
2. Без вступлений, предисловий, заключений. Сразу к сути.
3. НЕ расписывай план, сроки, дозировки подробно — только названия и суть.
4. НЕ объясняй ПОЧЕМУ — только ЧТО делать. Без агрономических обоснований.
5. НЕ перечисляй альтернативы — дай ОДНУ лучшую рекомендацию.
6. Объём: 60-100 слов. МАКСИМУМ 800 символов. Это примерно 4-6 строк текста.
7. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать больше 800 символов.
8. Завершающая фраза ровно одна строка: предложи узнать подробнее."""
...
КРИТИЧЕСКИ ВАЖНО: Весь ответ ТОЛЬКО про эту фазу ({phase_display}). Не упоминай работы других фаз даже кратко.
Один ответ = одна тема ({phase_topic}). НЕ смешивай темы.
...
2. **Конкретный план работ** для этой фазы — с датами, дозировками, названиями препаратов/удобрений.
3. **Обоснование** — почему именно в эти сроки и именно эти средства.
4. **Практические нюансы** — погодные условия, совместимость обработок, частые ошибки.
5. **Пошаговая последовательность** — что за чем, с какими интервалами.

ТРЕБОВАНИЯ К ОБЪЁМУ:
- МИНИМУМ 500 слов, оптимально 600-800 слов.
- МИНИМУМ 3500 символов текста.
- Каждый пункт — минимум 3-5 предложений.
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать меньше 3500 символов."""
...
КРИТИЧЕСКИ ВАЖНО: Ты отвечаешь ТОЛЬКО на ОДНУ фазу. Не упоминай и не описывай работы для других фаз.
Если пользователь спрашивает про весь сезон — всё равно отвечай ТОЛЬКО про одну фазу.
...
4. **Практические нюансы** — погодные условия, совместимость обработок, частые ошибки.
5. **Пошаговая последовательность** — что за чем, с какими интервалами.
6. ЗАВЕРШИ ответ фразой: "Если хотите, я дам рекомендации для следующей фазы роста."
...
Пользователь ЗАПЛАТИЛ за РАСШИРЕННЫЙ ответ. Он ожидает ЗНАЧИТЕЛЬНО больше информации чем в стандартном.
Ответ должен быть В 5-8 РАЗ длиннее стандартного. Это принципиально другой формат.

ОБЯЗАТЕЛЬНАЯ СТРУКТУРА (все пункты обязательны):
1. **Суть проблемы/вопроса** — 2-3 предложения, объясни контекст.
2. **Основная рекомендация** — подробно, с конкретными препаратами/дозировками/сроками.
3. **Альтернативные варианты** — минимум 2 альтернативы с плюсами и минусами каждой.
4. **Агрономическое обоснование** — объясни ПОЧЕМУ именно так, какие процессы происходят в растении.
5. **Частые ошибки** — что обычно делают неправильно и к чему это приводит.
6. **Практические нюансы** — погодные условия, совместимость, сроки ожидания, особенности для региона.
7. **Итоговая рекомендация** — конкретный пошаговый план действий.

ТРЕБОВАНИЯ К ОБЪЁМУ:
- МИНИМУМ 500 слов, оптимально 600-800 слов.
- МИНИМУМ 3500 символов текста.
- Каждый пункт структуры — минимум 3-5 предложений.
- Используй подзаголовки (выделяй жирным), нумерованные и маркированные списки.
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать меньше 3500 символов — пользователь заплатил за подробный ответ."""
