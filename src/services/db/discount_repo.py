...
async def upsert_broadcast_discount(
    ...
    bonus_tokens_mode: 'absolute' — фиксированное число токенов,
                       'percent' — процент от tokens_included тарифа.
    """
    pool = get_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            INSERT INTO user_broadcast_discounts
                (user_id, broadcast_id, option_key, discount_percent, bonus_tokens, bonus_tokens_mode, expires_at)
            VALUES
                ($1, $2, $3, $4, $5, $7, NOW() + ($6 * INTERVAL '1 hour'))
...
async def get_user_active_broadcast_discount(user_id: int) -> Optional[Dict[str, Any]]:
    ...
    Возвращает: discount_percent, bonus_tokens, bonus_tokens_mode, expires_at, broadcast_id
    """
    pool = get_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT discount_percent, bonus_tokens, COALESCE(bonus_tokens_mode, 'absolute') AS bonus_tokens_mode,
                   expires_at, broadcast_id
