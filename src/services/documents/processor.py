...
"""
Модуль обработки PDF-документов для загрузки в базу знаний.

Основная функция: process_pdf_document()
...
async def generate_embeddings_batch(texts: List[str]) -> List[List[float]]:
    ...
    Возвращает список embeddings той же длины, что и входной список.
    """
    embeddings = []
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    # В случае полной неудачи возвращаем нулевой вектор
...
async def process_pdf_document(
    ...
    """
    Обрабатывает PDF-документ: извлекает текст, разбивает на чанки,
    ...
    print(f"[process_pdf_document] Processing: {filename}")
    ...
    try:
        ...
        print(f"[process_pdf_document] Created document record ID: {document_id}")
    except Exception as e:
    ...
    try:
        ...
        print(f"[process_pdf_document] Created {len(chunks)} chunks")
    except Exception as e:
    ...
    try:
        print(f"[process_pdf_document] Generating embeddings for {len(chunk_data_list)} chunks...")
        all_embeddings = []
        ...
        for i in range(0, len(chunk_data_list), EMBEDDING_BATCH_SIZE):
            ...
            print(f"[process_pdf_document] Processing batch {i // EMBEDDING_BATCH_SIZE + 1}/{(len(chunk_data_list) + EMBEDDING_BATCH_SIZE - 1) // EMBEDDING_BATCH_SIZE}")
        ...
        print(f"[process_pdf_document] Generated {len(all_embeddings)} embeddings")
    ...
    try:
        ...
        print(f"[process_pdf_document] Inserted {len(chunks_for_db)} chunks into DB")
    except Exception as e:
        await document_update_status(
            document_id,
            status="failed",
    ...
    # 13. Обновление статуса документа на 'completed'
    try:
        await document_update_status(
            document_id,
            status="completed",
        ...
        print(f"[process_pdf_document] Document {document_id} processing completed")
    except Exception as e:
