...
def _keyword_fallback(raw_text: str) -> str:
    ...
    # Клубника / земляника
    if "земляник" in text and "клубник" not in text:
        candidates.add("земляника садовая")
    if "клубник" in text:
        candidates.add("клубника садовая")

    # Малина
    if "малин" in text:
        candidates.add("малина")

    # Смородина
    if "смородин" in text:
        if "черн" in text:
            candidates.add("чёрная смородина")
        if "красн" in text:
            candidates.add("красная смородина")
        if not any(c in candidates for c in ("чёрная смородина", "красная смородина")):
    ...
    if "жимолост" in text:
        if "съедобн" in text:
            candidates.add("жимолость съедобная")
        else:
            candidates.add("жимолость")

    # Ежевика
    if "ежевич" in text or "ежевик" in text:
        candidates.add("ежевика")

    # Черноплодная рябина / арония
    if "черноплодн" in text and "рябин" in text:
        candidates.add("черноплодная рябина")
    if "арони" in text:
        candidates.add("черноплодная рябина")
...
async def detect_culture_name(text: str) -> str:
    ...
    system_prompt = (
        ...
        "  3) Если вопрос даёт общие рекомендации сразу по нескольким культурам\n"
        "     или не привязан к одной — выбери: общая информация.\n"
        "  4) Если вопрос вообще не про ягодные культуры — выбери: не определено.\n"
    )
    ...
    try:
        ...
        mapping: Dict[str, str] = {
            "клубника садовая": "клубника садовая",
            "клубника": "клубника садовая",
            "земляника садовая": "земляника садовая",
            "земляника": "земляника садовая",
            "ремонтантная клубника": "клубника садовая",
            "ремонтантная земляника": "земляника садовая",
            "малина": "малина",
            "смородина": "смородина",
            "черная смородина": "чёрная смородина",
            "чёрная смородина": "чёрная смородина",
            "черная": "чёрная смородина",
            "красная смородина": "красная смородина",
            "красная": "красная смородина",
            "голубика": "голубика",
            "жимолость": "жимолость",
            "жимолость съедобная": "жимолость съедобная",
            "ежевика": "ежевика",
